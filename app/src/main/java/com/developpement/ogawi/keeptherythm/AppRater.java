...
public class AppRater {
    ...
    public static void app_launched_after100(Context mContext) {
        //est lancé  une seule fois lorsque le joueur atteint les 100%
        SharedPreferences prefs = mContext.getSharedPreferences("apprater", 0);
        if (prefs.getBoolean("dontshowagain", false)) { return ; }
        ...
        if (launch_count >= LAUNCHES_UNTIL_PROMPT) {
            if (System.currentTimeMillis() >= date_firstLaunch +
                    (DAYS_UNTIL_PROMPT * 24 * 60 * 60 * 1000)) {
                showRateDialog_after100(mContext, editor);
            }
        }
    ...
    public static void showRateDialog(final Context mContext, final SharedPreferences.Editor editor) {
        ...
        dialog.setTitle("Noter " + APP_TITLE);
    ...
    public static void showRateDialog_after100(final Context mContext, final SharedPreferences.Editor editor) {
        final Dialog dialog = new Dialog(mContext);
        dialog.setTitle("Noter " + APP_TITLE);
        ...
        ll.addView(emoj);
        TextView tv = new TextView(mContext);
        tv.setText("Félicitations, tu a terminé tous les niveaux du jeu.Tu es arrivé là où peu ont eu le talent pour y arriver.Tu a prouvé que tu a le rythme dans la peau.Si tu a apprécié jouer à " + APP_TITLE + ", prends un moment pour le noter. Merci pour le soutien!");
        tv.setTextSize(17);
        tv.setTextColor(mContext.getResources().getColor(R.color.black));
        tv.setWidth(700);
        tv.setPadding(7, 0, 4, 10);
